...
const EerResult = () => {
  ...
  const getWhatDoesThisMean = (eer: number): JSX.Element => {
    if (eer < 1200) {
      return (
        <>
  ...
  return (
    <div className="flex flex-col gap-6 w-full items-center justify-center min-h-screen text-center p-4 bg-gradient-to-br from-slate-50 via-blue-50 to-teal-50">
      ...
      >
        ...
        <span className="text-teal-600">{eerResult?.toFixed(0)}</span> kcal/day
      </motion.h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl w-full mb-8">
        ...
        >
          ...
          <div className="text-gray-700 space-y-3 text-left">
            {getWhyDoesThisMatter(eerResult)}
          </div>
        </motion.div>
      </div>
      ...
      >
        <Link to="/eer" className="btn-primary inline-flex items-center gap-2">
          Back to EER Calculator <FaArrowRightLong />
        </Link>
      </motion.div>
    </div>
  );
};
export default EerResult;
